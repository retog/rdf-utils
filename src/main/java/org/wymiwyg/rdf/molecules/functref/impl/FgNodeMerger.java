...
public class FgNodeMerger {
	...
	static class ResultMap<T> extends HashMap<T, FunctionallyGroundedNode> {
		 ...
		 */
		public ResultMap(Map<T, FunctionallyGroundedNode> indexedOrigFgNodes) {
			...
			// this is because fgnodes may be multiple times in the map alredy
			// at
			// the beginning
			...
			Collection<FgNodeRoleDescriptor> descriptorList = nodes2Roles
					.values();
			...
			if (log.isDebugEnabled()) {
				log.debug("Summary: original size: "
						+ keySet().size()
						+ " new size "
						+ new HashSet<FunctionallyGroundedNode>(values())
								.size());
				log.debug("result");
				int debCounter = 0;
			...
			verifyResultConsistency(new HashSet<FunctionallyGroundedNode>(
					values()));
		...
		/**
		 * This is to remove overlapping fgnodes, i.e. fgnodes representing the
		 * same node because they share an nt-molecule
		 * 
		 * @param result
		 */
		/*
		 * iterate throw fg-nodes, for each nt-molecule that is present in
		 * another fg-node, add the other fg-node to a merge-set, if merge-set >
		 * 0, add curent fg-node to merge-set and create a new merged fg node.
		 * look up key for every value in merge-set and set the value in
		 * origAfgn2FgnMap to the new fgnode.
		 */
		/*
		 * map nt-molecules to a set of map entries of origAfgn2FgnMap in which
		 * with value-fgnode containing that molecule. iterate through the map,
		 * and where the set > 1, and the values (fgnodes) are different merge
		 * them, and set the value of the inner entry accordingly. start over
		 * (or could this done more selectively:
		 */
		/*
		 * the fg node are put in a list of FgNodeRoleDescriptor, for every
		 * element it is checked against all previuos elements, when a merge is
		 * done, the current elment is removed and the algorithm restarts at the
		 * element following the merged element.
		 */
		...
		private boolean replaceNodesInNodes(
			...
			for (FunctionallyGroundedNode node : nodes) {
				for (NonTerminalMolecule ntMolecule : node
						.getGroundingMolecules()) {
					for (Triple triple : ntMolecule) {
						...
						if (object instanceof FunctionallyGroundedNode) {
							if (!nodes.contains(object)) {
								log.warn("object in OrigNodes: "
										+ origNodes.contains(object));
		...
		private boolean replaceFgNode(
				FunctionallyGroundedNodeImpl editableFgNode,
				FunctionallyGroundedNode source, FunctionallyGroundedNode target) {
			boolean result = false;
			...
			for (NonTerminalMolecule ntMolecule : editableFgNode
				...
				try {
					editableTripleSet = new GraphUtil<Set<Triple>>()
							.replaceNode(editableTripleSet, source, target,
									new HashSet<Triple>());
				...
				if (replaced) {
					if (log.isDebugEnabled()) {
						log.debug("replaced " + source + " with " + target
								+ " in " + editableFgNode);
		...
		/**
		 * checks the element at pos against previous elemnts, starting with 0,
		 * till a merge can be made
		 * 
		...
		private int checkAgainstPreviousOnList(
			...
			for (int i = 0; i < pos; i++) {
				if (shareMolecule(descriptor.fgNode,
		...
		private void mergeFgNodesDescriptor(FgNodeRoleDescriptor descriptor,
			...
			descriptor.fgNode = mergeFgNodes(descriptor.fgNode,
					descriptor2.fgNode);
		}
		...
		private static boolean shareMolecule(FunctionallyGroundedNode fgNode1,
			...
			Set<NonTerminalMolecule> molecules2 = fgNode2
					.getGroundingMolecules();
		...
		FunctionallyGroundedNodeImpl mergeFgNodes(
			 ...
			 * resultMolecules.addAll(fgNode2.getGroundingMolecules()); return
			 * new MergerFunctionallyGroundedNodeImpl(resultMolecules);
			 */
		}
		...
		private void verifyResultConsistency(
			...
			// for (NonTerminalMolecule ntMolecule :
